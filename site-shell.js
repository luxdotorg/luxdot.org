...
function purity(){
 ...
 document.querySelectorAll('body *').forEach(el=>{
  ...
  if(el.children.length===0&&foreign.test(el.textContent||'')){
    if((el.textContent||'').includes('رافي عبد السميع الحجي'))el.textContent=(el.textContent||'').replaceAll('رافي عبد السميع الحجي','Rafi Abd al-Samiʿ al-Hajji');
  }
 });
}

...
function addButtons(){
 ...
 targetNodes().forEach(el=>{
   if(el.querySelector(':scope > .lux-claim-actions'))return;
   const text=cleanText(el),id='claim-'+hash(location.pathname+'|'+text);if(!el.id)el.id=id;
   const wrap=document.createElement('span');wrap.className='lux-claim-actions';
   const defs=[
    ['correct','assets/icons/luxdot-symbols/correct.svg',t.types[0]||'تصحيح معلومة'],
    ['challenge','assets/icons/luxdot-symbols/challenge.svg',t.types[1]||'اعتراض على الاستنتاج'],
    ['report','assets/icons/luxdot-symbols/report.svg',t.types[3]||'رابط لا يعمل']
   ];
   defs.forEach(([mode,src,label])=>{
     const b=document.createElement('button');b.type='button';b.className='lux-challenge-btn lux-'+mode;
     b.innerHTML=`<img src="${src}" alt="">`;b.setAttribute('aria-label',label);b.setAttribute('title',label);
     b.addEventListener('click',e=>{e.stopPropagation();openModal(text,el.id,mode)});
     wrap.appendChild(b);
   });
   el.appendChild(wrap);
 });
}
function openModal(claim,anchor,mode='correct'){
 ...
 const sel=m.querySelector('.lux-issue-type');
 sel.selectedIndex=(mode==='challenge'?1:(mode==='report'?3:0));
 m.querySelector('.lux-evidence').value='';m.querySelector('.lux-note').value='';

...
const q=document.getElementById('careSearch'),cat=document.getElementById('careCat'),town=document.getElementById('careTown'),list=document.getElementById('careList'),count=document.getElementById('visibleCount'),legend=document.getElementById('careLegend'),frame=document.getElementById('careLeafletMap'),openMap=document.getElementById('careMapOpen');
if(!q||!cat||!town||!list||!count||!frame)return;
const lang=()=>{const query=new URLSearchParams(location.search).get('lang');if(['ar','en','nl','id','jv','he'].includes(query))return query;const raw=localStorage.getItem('luxdot.lang');return raw==='id'?'id':(window.LuxLang?.get?.()||'en')};
...
function mapUrl(p,zoom=13){const lat=Number(p.lat),lon=Number(p.lon),dx=zoom>=13?.08:.30,dy=zoom>=13?.06:.22;return `https://www.openstreetmap.org/export/embed.html?bbox=${(lon-dx).toFixed(5)}%2C${(lat-dy).toFixed(5)}%2C${(lon+dx).toFixed(5)}%2C${(lat+dy).toFixed(5)}&layer=mapnik&marker=${lat}%2C${lon}`}
function openUrl(p){return `https://www.openstreetmap.org/?mlat=${p.lat}&mlon=${p.lon}#map=14/${p.lat}/${p.lon}`}
function focusPoint(p){if(!p)return;frame.src=mapUrl(p,13);if(openMap)openMap.href=openUrl(p);frame.scrollIntoView({behavior:'smooth',block:'center'})}
function renderList(f){const u=ui();list.innerHTML=f.map(p=>`<article class="care-card" data-care-id="${p.id}"><button class="care-card-main" type="button" data-care-focus="${p.id}"><strong>${p.name}</strong><span>${p.town} · ${(Number(p.distance)||0).toFixed(1)} ${u.km}</span><small><i class="care-cat-dot" style="background:${color(p.category)}"></i>${catLabel(p.category)}</small></button>${p.url?`<a class="care-source" href="${p.url}" target="_blank" rel="noopener">${u.source} ↗</a>`:''}</article>`).join('');list.querySelectorAll('[data-care-focus]').forEach(b=>b.addEventListener('click',()=>focusPoint(points.find(p=>String(p.id)===b.dataset.careFocus))))}
function draw(){const f=fit();count.textContent=f.length;renderList(f);if(f.length===1)focusPoint(f[0])}
function initSelects(){const u=ui(),cv=cat.value,tv=town.value;cat.innerHTML=`<option value="">${u.allCats}</option>`;[...new Set(points.map(p=>p.category).filter(Boolean))].sort().forEach(c=>cat.add(new Option(catLabel(c),c)));town.innerHTML=`<option value="">${u.allTowns}</option>`;[...new Set(points.map(p=>p.town).filter(Boolean))].sort().forEach(t=>town.add(new Option(t,t)));cat.value=cv;town.value=tv}
function initLegend(){if(!legend)return;const cats=[...new Set(points.map(p=>p.category).filter(Boolean))];legend.innerHTML=cats.map(c=>`<span><i style="background:${color(c)}"></i>${catLabel(c)}</span>`).join('')}
function translate(){const u=ui();const title=document.getElementById('careTitle'),lead=document.getElementById('careLead'),pw=document.getElementById('pointsWord'),eb=document.querySelector('.care-head .eyebrow');if(title)title.textContent=u.title;if(lead)lead.textContent=u.lead;if(pw)pw.textContent=u.points;if(eb)eb.textContent=u.eyebrow;q.placeholder=u.search;document.title=`${u.title} · LuxDot`;document.documentElement.lang=lang();document.documentElement.dir=['ar','he'].includes(lang())?'rtl':'ltr';initSelects();initLegend();draw()}
function boot(){translate();[q,cat,town].forEach(x=>x.addEventListener(x===q?'input':'change',draw));if(points[0])focusPoint(points[0])}

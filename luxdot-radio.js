...
function load(t,play=true){
 current=t;sacredLock=t.kind==='sacred';audio.pause();audio.src=t.url;audio.load();updateDock();emit();
 if(play&&userOn)audio.play().catch(()=>setStatus('اضغط تشغيل للسماح بالصوت · Tap play to allow audio'));
}

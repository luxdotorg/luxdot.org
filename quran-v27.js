...
async function loadSurah(s,a=1){
  ...
  list($("#sfilter").value||"");
  try{
    const editions={en:"en.sahih",id:"id.indonesian"}; const edition=editions[LANG];
    const url=edition?`${QAPI}${s}/editions/quran-uthmani,${edition}`:`${QAPI}${s}/quran-uthmani`;
    const r=await fetch(url,{cache:"no-store"});
    if(!r.ok)throw new Error("HTTP "+r.status);
    const j=await r.json();
    const packet=Array.isArray(j.data)?j.data:[j.data]; const ar=packet[0].ayahs,en=packet[1]?.ayahs||[];
    state.data=ar.map((v,i)=>({n:v.numberInSurah,text:v.text,en:en[i]?.text||""}));
    const trans=$("#trans"); if(trans){trans.hidden=!edition;if(!edition){state.en=false;trans.classList.remove("on")}}
    render();
  }catch(e){

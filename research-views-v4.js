...
const labels={ar:{title:'طريقة عرض الأبحاث',hint:'أربع عدسات لنفس المعرفة',sky:'السماء',universe:'الكون',neural:'الشبكة العصبية',motherboard:'اللوحة الأم',galaxy:'مجرة',cluster:'عنقود',system:'نظام',planet:'كوكب',moon:'قمر',asteroid:'كويكب'},en:{title:'Research views',hint:'Four lenses on the same knowledge',sky:'SKY',universe:'UNIVERSE',neural:'NEURAL NETWORK',motherboard:'MOTHERBOARD',galaxy:'Galaxy',cluster:'Cluster',system:'System',planet:'Planet',moon:'Moon',asteroid:'Asteroid'},nl:{title:'Onderzoeksweergaven',hint:'Vier lenzen op dezelfde kennis',sky:'HEMEL',universe:'HEELAL',neural:'NEURAAL NETWERK',motherboard:'MOEDERBORD',galaxy:'Melkweg',cluster:'Cluster',system:'Systeem',planet:'Planeet',moon:'Maan',asteroid:'Asteroïde'},he:{title:'תצוגות מחקר',hint:'ארבע עדשות על אותו ידע',sky:'שמיים',universe:'יקום',neural:'רשת עצבית',motherboard:'לוח אם',galaxy:'גלקסיה',cluster:'צביר',system:'מערכת',planet:'כוכב לכת',moon:'ירח',asteroid:'אסטרואיד'}};
...
function neural(){classify();const eg=$('.rg-constellations');if(eg){const layer=svgEl('g',{class:'rg-neural-layer'});nodes().forEach(n=>{const p=parsePos(n);if(p)layer.append(svgEl('circle',{cx:p[0],cy:p[1],r:24,class:'rg-neuron-ring'}))});eg.parentNode.insertBefore(layer,eg)}nodes().forEach(n=>n.classList.add('rg-role-neuron'));legend(['NEURON = RESEARCH','SYNAPSE = RELATION','PULSE = ACTIVE CONNECTION'])}
...
function apply(newMode,persist=true){
 ...
 document.querySelectorAll('.rg-symbolism [data-symbol]').forEach(x=>x.classList.toggle('active',x.dataset.symbol===mode));badge();tabs();if(selected)requestAnimationFrame(()=>pulse(selected));
}
function ready(){

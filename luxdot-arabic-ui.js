...
function blankBook(){const trigger=document.querySelector('[data-blank-book]');if(!trigger)return;const l=(new URLSearchParams(location.search).get('lang')||localStorage.getItem('luxdot.lang')||'en').toLowerCase(),X={ar:['إغلاق الكتاب والعودة إلى المكتبة','تُرك هذا الكتاب فارغاً عمداً.','ليس لأن الحكاية انتهت، بل لأنها لم تُكتب بعد.','هذه الصفحات للأجيال القادمة؛ لتكتب ما عرفته وما صححته وما رأت أننا لم نره.','لا ترثوا أجوبتنا فقط. اتركوا مكاناً لأجوبتكم.'],en:['Close the book and return to the library','This book was intentionally left blank.','Not because the story ended, but because it has not yet been written.','These pages belong to future generations: to write what they learned, corrected and saw that we could not.','Do not inherit only our answers. Leave room for yours.'],nl:['Sluit het boek en keer terug naar de bibliotheek','Dit boek is bewust leeg gelaten.','Niet omdat het verhaal voorbij is, maar omdat het nog niet is geschreven.','Deze pagina’s zijn voor toekomstige generaties: voor wat zij leren, corrigeren en zien wat wij niet zagen.','Erf niet alleen onze antwoorden. Laat ruimte voor die van jullie.'],he:['סגירת הספר וחזרה לספרייה','הספר הזה הושאר ריק בכוונה.','לא מפני שהסיפור הסתיים, אלא מפני שהוא עדיין לא נכתב.','הדפים האלה שייכים לדורות הבאים: לכתוב מה למדו, תיקנו וראו שאנחנו לא ראינו.','אל תרשו רק את התשובות שלנו. השאירו מקום לתשובות שלכם.'],fr:['Fermer le livre et revenir à la bibliothèque','Ce livre a été laissé vide volontairement.','Non parce que l’histoire est finie, mais parce qu’elle n’est pas encore écrite.','Ces pages appartiennent aux générations futures.','N’héritez pas seulement de nos réponses.'],es:['Cerrar el libro y volver a la biblioteca','Este libro fue dejado en blanco deliberadamente.','No porque la historia haya terminado, sino porque aún no ha sido escrita.','Estas páginas pertenecen a las generaciones futuras.','No hereden solo nuestras respuestas.'],de:['Buch schließen und zur Bibliothek zurückkehren','Dieses Buch wurde absichtlich leer gelassen.','Nicht weil die Geschichte beendet ist, sondern weil sie noch nicht geschrieben wurde.','Diese Seiten gehören künftigen Generationen.','Erbt nicht nur unsere Antworten.'],tr:['Kitabı kapat ve kütüphaneye dön','Bu kitap bilerek boş bırakıldı.','Hikâye bittiği için değil, henüz yazılmadığı için.','Bu sayfalar gelecek kuşaklarındır.','Yalnızca cevaplarımızı miras almayın.'],id:['Tutup buku dan kembali ke perpustakaan','Buku ini sengaja dibiarkan kosong.','Bukan karena kisahnya berakhir, tetapi karena belum ditulis.','Halaman-halaman ini untuk generasi mendatang.','Jangan hanya mewarisi jawaban kami.'],jv:['Tutup buku lan bali menyang pustaka','Buku iki sengaja ditinggal kosong.','Dudu amarga critane wis rampung, nanging amarga durung ditulis.','Kaca-kaca iki kanggo generasi mbesuk.','Aja mung marisi jawaban kita.']},t=X[l]||X.en,ov=document.createElement('div');ov.className='lux-blank-overlay';ov.innerHTML=`<button type="button" class="lux-book-close">${t[0]}</button><div class="lux-blank-spread"><div class="lux-blank-page"></div><div class="lux-blank-page lux-future-page"><div class="lux-future-note"><strong>${t[1]}</strong><span>${t[2]}</span><span>${t[3]}</span><em>${t[4]}</em></div></div></div>`;document.body.appendChild(ov);const close=ov.querySelector('button');trigger.addEventListener('click',()=>{photonSwarm(trigger);ov.classList.add('on')});close.addEventListener('click',()=>{stopAllAudio();ov.classList.remove('on');trigger.focus()});ov.addEventListener('click',e=>{if(e.target===ov){stopAllAudio();ov.classList.remove('on')}})}

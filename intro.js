(()=>{"use strict";
const C=document.getElementById('matrix'),x=C.getContext('2d'),signals=document.getElementById('signals'),beliefStars=document.getElementById('beliefStars'),enter=document.getElementById('enter'),sound=document.getElementById('sound'),sub=document.getElementById('sub'),eyebrow=document.getElementById('eyebrow'),micro=document.getElementById('micro'),burst=document.getElementById('burst'),statusA=document.getElementById('statusA'),statusB=document.getElementById('statusB'),languageSelect=document.getElementById('languageSelect');
...
function setLang(l,selected=false){if(!L[l])return;lang=l;if(selected)persistLang(l);document.documentElement.lang=l;document.documentElement.dir=L[l].dir;sub.textContent=L[l].sub;eyebrow.textContent=L[l].eye;enter.textContent=L[l].enter;micro.textContent=L[l].micro;statusA.textContent=L[l].a;statusB.textContent=L[l].b;if(pilotNote)pilotNote.textContent=PILOT[l]||PILOT.en;if(languageSelect&&languageSelect.value!==l)languageSelect.value=l}
if(languageSelect)languageSelect.addEventListener('change',e=>{rememberPoint(e);startAmbient();setLang(e.target.value,true)});
...
function scheduleSOS(){clearTimeout(morseTimer);const duration=playMorseMessage();morseTimer=setTimeout(scheduleSOS,duration+1000)}
function startAmbient(){if(on)return;const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;audioCtx=new AC();master=audioCtx.createGain();master.gain.value=.0001;master.connect(audioCtx.destination);const low=audioCtx.createOscillator(),lg=audioCtx.createGain(),fifth=audioCtx.createOscillator(),fg=audioCtx.createGain();low.type='sine';low.frequency.value=43;lg.gain.value=.014;fifth.type='sine';fifth.frequency.value=64.5;fg.gain.value=.003;low.connect(lg).connect(master);fifth.connect(fg).connect(master);low.start();fifth.start();master.gain.exponentialRampToValueAtTime(.92,audioCtx.currentTime+.45);nodes=[low,fifth];on=true;sound.classList.add('on');scheduleSOS()}
function stopAmbient(){if(!on)return;clearTimeout(morseTimer);master.gain.exponentialRampToValueAtTime(.0001,audioCtx.currentTime+.16);setTimeout(()=>{nodes.forEach(n=>{try{n.stop()}catch{}});try{audioCtx.close()}catch{};on=false;nodes=[];sound.classList.remove('on')},190)}
sound.addEventListener('click',e=>{e.stopPropagation();rememberPoint(e);on?stopAmbient():startAmbient()});function rememberPoint(e){if(e&&Number.isFinite(e.clientX))lastPoint={x:e.clientX,y:e.clientY}}
let activationArmed=true;
function activateSignal(e){if(e)rememberPoint(e);try{if(!on)startAmbient();if(audioCtx&&audioCtx.state==='suspended'){audioCtx.resume().then(()=>{activationArmed=false}).catch(()=>{activationArmed=true})}else activationArmed=false}catch(_){activationArmed=true}}
// Try immediately. Browsers that allow autoplay will begin before any language choice.
setTimeout(()=>{try{startAmbient();if(audioCtx&&audioCtx.state==='suspended')audioCtx.resume().catch(()=>{activationArmed=true})}catch(_){activationArmed=true}},80);
// If autoplay is blocked, arm every genuine user-activation route. Pointer movement is
// attempted as requested; pointerdown/touch/key are the standards-compliant fallback.
document.addEventListener('pointermove',activateSignal,{once:true,passive:true});
document.addEventListener('mousemove',activateSignal,{once:true,passive:true});
document.addEventListener('pointerdown',e=>{if(e.target!==sound)activateSignal(e)},{once:true,passive:true});
document.addEventListener('touchstart',activateSignal,{once:true,passive:true});
document.addEventListener('keydown',activateSignal,{once:true});
function cross(e){rememberPoint(e);persistLang(lang);if(!on)startAmbient();setTimeout(stopAmbient,40);clarity=1;statusA.textContent=L[lang].acq;statusB.textContent=L[lang].mem;const px=(lastPoint.x/innerWidth*100).toFixed(2),py=(lastPoint.y/innerHeight*100).toFixed(2);try{sessionStorage.setItem('luxdot.entry.point',JSON.stringify({x:px,y:py}))}catch{}burst.style.setProperty('--bx',lastPoint.x+'px');burst.style.setProperty('--by',lastPoint.y+'px');burst.classList.remove('go');void burst.offsetWidth;burst.classList.add('go');document.documentElement.style.setProperty('--crt-x',lastPoint.x+'px');document.documentElement.style.setProperty('--crt-y',lastPoint.y+'px');document.body.classList.add('crt-out');const path=localStorage.getItem('luxdot.agePath')==='kids'?'kids-galaxy.html':'home.html';setTimeout(()=>location.href=path+'?lang='+encodeURIComponent(lang),760)}enter.addEventListener('click',cross);
})();
